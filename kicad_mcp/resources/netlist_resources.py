...
def register_netlist_resources(mcp: FastMCP) -> None:
    ...
    def get_netlist_resource(schematic_path: str) -> str:
        ...
        """
        logger.info(f"Generating netlist report for schematic: {schematic_path}")
        ...
        try:
            ...
            report += "- **Netlist Quality**: Partial connectivity inference\n"

            if "total_pin_connections" in analysis_results:
                report += f"- **Pin Connections**: {analysis_results['total_pin_connections']}\n"

            report += "\n"
            report += "> Warning: connectivity tracing is currently incomplete. Nets are inferred from global labels and power symbols only.\n\n"
    ...
    def get_project_netlist_resource(project_path: str) -> str:
        ...
        Returns:
            Markdown-formatted netlist report
        """
        logger.info(f"Generating netlist report for project: {project_path}")
    ...
    def get_component_resource(schematic_path: str, component_ref: str) -> str:
        ...
        """
        logger.info(
            f"Generating component report for {component_ref} in schematic: {schematic_path}"
        )
