        ...
        for file_type, file_path in bom_files.items():
            try:
                ...
                if not bom_data or len(bom_data) == 0:
                    logger.info(f"Failed to parse BOM file: {file_path}")
                    continue
                ...
                logger.info(f"Error analyzing BOM file {file_path}: {str(e)}", exc_info=True)
    ...
    try:
        if ext == ".csv":
            ...
            try:
                ...
                logger.info(f"Failed to parse unknown file format: {file_path}")
                return [], {"detected_format": "unsupported"}

    except Exception as e:
        logger.info(f"Error parsing BOM file: {str(e)}", exc_info=True)
        return [], {"error": str(e)}

    # Check if we actually got components
    if not components:
        logger.info(f"No components found in BOM file: {file_path}")
    else:
    ...
    try:
        ...
        if cost_col:
            try:
                ...
                logger.info("Failed to parse cost data")
        ...
        logger.info(f"Error analyzing BOM data: {str(e)}", exc_info=True)
...
async def export_bom_with_cli(
    ...
    try:
        ...
        if process.returncode != 0:
            logger.error("BOM export command failed with code %s", process.returncode)
