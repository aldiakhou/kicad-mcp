...
def register_netlist_tools(mcp: FastMCP) -> None:
    ...
    async def extract_project_netlist(project_path: str, ctx: Context | None) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Error extracting project netlist: {str(e)}")
            if ctx:
    ...
    async def analyze_schematic_connections(
        ...
        try:
            ...
            logger.exception("Error analyzing connections: %s", e)
            if ctx:
    ...
    async def find_component_connections(
        ...
        try:
            ...
            if component_ref not in components:
                logger.info(f"Component {component_ref} not found in schematic")
                if ctx:
            ...
            logger.info(f"Error finding component connections: {str(e)}", exc_info=True)
            if ctx:
